...
@section('headerTitle', 'Member KYC Verification')
@section('mycss')
    <style>
        .custom-btn {
            padding: 6px 10px;
                                        ...
                                        <tbody class="table-border-bottom-0">
                                            ...
                                            @forelse ($kyc_pending as $item)
                                                <tr id="trRow{{ $item->id }}">
                                                    ...
                                                    <td>
                                                        {{ \Carbon\Carbon::parse($item->registered_at)->format('d F Y') }}
                                                    </td>

                                                    <td>
                                                        <a href="javascript:void(0)" data-id="{{ $item->id }}"
                                                            title="DETAILS" data-toggle="modal" data-bs-toggle="modal"
                                                            data-bs-target="#kycVerificationDetails"
                                                            class="custom-btn search-btn"" target="_blank">
        ...
        <script>
            $(document).ready(function() {
                ...
                $(document).on('click', "a[data-bs-target='#kycVerificationDetails']", function() {
                    ...
                    var formUrl = "{{ url('admin/pending-member-kyc-verification-details') }}" + "/" + id;
